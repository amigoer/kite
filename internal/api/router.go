...
func registerAPIRoutes(router *gin.Engine, cfg *config.Config, db *gorm.DB) {
	...
	adminV1.GET("/friend-links", friendLinkHandler.List)
	adminV1.GET("/friend-links/:id", friendLinkHandler.GetByID)
	adminV1.POST("/friend-links", friendLinkHandler.Create)
	adminV1.PUT("/friend-links/:id", friendLinkHandler.Update)
	adminV1.PATCH("/friend-links/:id", friendLinkHandler.Patch)
	adminV1.DELETE("/friend-links/:id", friendLinkHandler.Delete)
	adminV1.GET("/tags", tagHandler.List)
	adminV1.GET("/tags/:id", tagHandler.GetByID)
	adminV1.POST("/tags", tagHandler.Create)
	adminV1.PUT("/tags/:id", tagHandler.Update)
	adminV1.PATCH("/tags/:id", tagHandler.Patch)
	adminV1.DELETE("/tags/:id", tagHandler.Delete)
	adminV1.GET("/categories", categoryHandler.List)
	adminV1.GET("/categories/:id", categoryHandler.GetByID)
	adminV1.POST("/categories", categoryHandler.Create)
	adminV1.PUT("/categories/:id", categoryHandler.Update)
	adminV1.PATCH("/categories/:id", categoryHandler.Patch)
	adminV1.DELETE("/categories/:id", categoryHandler.Delete)
}
