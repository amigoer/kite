...
func registerAPIRoutes(router *gin.Engine, cfg *config.Config, db *gorm.DB) {
	...
	commentLimiter := NewRateLimiter(5, 10*time.Minute)
	apiV1.GET("/posts/:id/comments", commentHandler.ListByPost)
	apiV1.POST("/posts/:id/comments", commentLimiter.Middleware(), commentHandler.Create)
	...
	protectedAdminV1.PUT("/profile/password", profileHandler.ChangePassword)
