...
func Open(ctx context.Context, path string) (*Store, error) {
	...
	// Schema migrations for existing databases. Idempotent: errors are
	// expected on the "happy path" (column already removed) and ignored.
	for _, ddl := range []string{
		`ALTER TABLE rooms DROP COLUMN mode`,
	} {
		if _, err := db.ExecContext(ctx, ddl); err != nil {
			// Ignore "no such column" — already dropped.
		}
	}
