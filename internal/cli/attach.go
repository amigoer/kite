...
func newShellCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Spawn a fresh room and attach to it. The room runs bash with the
kite prompt sentinel installed so both interactive use and structured
kite exec calls work against the same shell.`,
	...
	cmd.Flags().StringVar(&shellPath, "shell", "", "shell binary (defaults to /bin/bash)")
	return cmd
}
