...
function renderWithBold(
  ...
  return template.split(re).map((chunk, i) =>
    keys.includes(chunk) ? (
...
function isDocument(file: { file_type: string; original_name: string }): boolean {
...
export default function FilesPage() {
  ...
  const { data: fileBucketSample } = useQuery<{ items: FileItem[]; total: number }>({
                  ...
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="mt-0.5 flex items-center gap-2 text-[11px] tabular-nums text-muted-foreground">
                        ...
                        <span>{formatBytes(file.size_bytes)}</span>
                        <span className="text-muted-foreground/60">·</span>
                        <span>{formatRelativeTime(file.created_at, locale)}</span>
                      </div>
                    </div>
                    {!anySelected && (
              ...
              <div className="space-y-2">
                ...
                {[
                  ...
                  ...(detailFile.source_url
                    ? [{ label: t("files.sourceUrl"), value: detailFile.source_url }]
                    : []),
                  {
                    label: "Markdown",
